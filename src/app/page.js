...
export default async function Home({ searchParams }) {
  const searchBy = await searchParams.searchBy || "title";
  const search = await searchParams.search || "";
  const page = parseInt(await searchParams.page) || 1;
  const sortBy = await searchParams.sortBy || "id";
  const sortOrder =
    await searchParams.sortOrder && await searchParams.sortOrder.toLowerCase() === "asc"
      ? "asc"
      : "desc";
  ...
  try {
    ...
    return (
      <ClientContentList
        initialContents={contentsWithFormattedDate}
        currentPage={page}
